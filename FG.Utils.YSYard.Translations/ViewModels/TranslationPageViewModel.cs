...
public class TranslationPageViewModel : ComponentBase, IDisposable
{
	...
	[Inject]
	protected IFormMessengerService FormMessenger
	{
		get; set;
	} = null!;
    ...
    protected void SetPluginFolder()
	{
		this.FormMessenger.RequestFolderPath(this.JSRuntime, x =>
		{
			...
			this.PluginFolderPath = x;
			this.PluginIsFound = new LanguagePathDefs(x).IsValid;
			this.LanguageRepository.SetPluginFolderPath(x);
			this.TranslationRepository.SetPluginFolderPath(x);
    ...
    protected void SaveBackup()
    {
        ...
        this.FormMessenger.RequestSaveFilePath(this.JSRuntime, async x =>
        {
            var json = await this.TranslationRepository.SaveBackupAsync(x).ConfigureAwait(false);
            this.Snackbar.Add("保存完了", Severity.Info);
        }, opts);
    }

    protected virtual void LoadBackup()
    {
        if (!this.PluginIsFound)
        {
            this.Snackbar.Add("プラグインフォルダがセットされていないか, 有効ではありません.", Severity.Error);
        ...
        var opts = new FilePickerOptions();
        opts.Filters["JSONファイル"] = [".json"];
        this.FormMessenger.RequestOpenFilePath(this.JSRuntime, async x =>
        {
            await this.TranslationRepository.LoadBackupAsync(x).ConfigureAwait(false);
            this.Snackbar.Add("上書き完了", Severity.Info);
        }, opts);
    }

    protected async Task SerializeTranslationsAsync()
    {
        if (!this.PluginIsFound)
        {
            this.Snackbar.Add("プラグインフォルダがセットされていないか, 有効ではありません.", Severity.Error);
        ...
        var res = await this.TranslationRepository.SerializeAsync();
        if (res)
        {
            this.Snackbar.Add("保存完了", Severity.Info);
        }
